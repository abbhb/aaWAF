...
func AddSiteJson(siteId string, domain []string, ipList []string, siteName string, isHttps bool, fullChain string, privateKey string, cdn bool, pollingAlgorithm string, hostStr string, addPort []string, addMap []map[string]string) error {
	...
	locationNot := AddLocationJson("", "/", proxyPass, "1.1", []string{"Host", "Upgrade $http_upgrade", "Connection \"upgrade\"", "X-Real-IP $remote_addr", "X-Forwarded-For $proxy_add_x_forwarded_for", "X-Forwarded-Proto $http_x_forwarded_proto"}, []string{"error timeout invalid_header http_500 http_502 http_503 http_504"}, "off", "", "", nil, "", "", "", "")
